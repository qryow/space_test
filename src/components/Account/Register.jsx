            ...
            <div className={style.container}>
              ...
              <div className={style.block__wrapper}>
                <div className={style.block}>
                  <h2 className={style.block__title}>Create an account</h2>
                ...
                <div className={style.pass__fields}>
                  <div className={style.pass__field}>
                    ...
                    <div className={style.input__wrapper}>
                      <input type={showPassword ? 'text' : 'password'} className={style.pass__input} onChange={(e) => setUserObj({ ...userObj, password: e.target.value })} />
                      <img onClick={togglePasswordVisibility} className={style.hide} src={showPassword ? hide : hide} alt="" />
                    </div>
                  </div>

                  <div className={style.pass__field}>
                    <p className={style.input__title}>Confirm your password</p>
                      <div className={style.input__wrapper}>
                        <input type={showPassword ? 'text' : 'password'} className={style.pass__input} onChange={(e) => setUserObj({ ...userObj, password_confirm: e.target.value })} />
                        <img onClick={togglePasswordVisibility} className={style.hide} src={showPassword ? hide : 'open'} alt="" />
                      </div>
                    </div>
                  </div>
                  <p className={style.pass__sub}>Use 8 or more characters with a mix of letters, numbers & symbols</p>
                  <div className={style.privacy}>
                    <input className={style.checkbox} type="checkbox" />
                    <p className={style.privacy__text}>Agree to our <a href="#">Terms of use</a> and <a href="#">Privacy Policy</a> </p>
                  </div>

                  <button className={style.reg__btn} onClick={() => {dispatch(registerUser({ userObj, navigate })); addEmail()} }>Create Account</button>
                </div>
              </div>
            </div>
          </div>
                    ...
                    <div className={style.container}>
                      ...
                      <div className={style.block__wrapper}>
                        <div className={style.block}>
                          ...
                          <div className={style.pass__fields}>
                            <div className={style.pass__field}>
                              <p className={style.input__title}>{fullPass ? <span className={style.error}> {fullPass} </span> : 'Password'}</p>
                              <div className={style.input__wrapper}>
                                <input type={showPassword ? 'text' : 'password'} placeholder={passwordError ? passwordError : ''} className={passwordError || passwordMessage || fullPass ? `${style.error__input} ${style.pass__input}` : `${style.pass__input}`} onChange={(e) => setUserObj({ ...userObj, password: e.target.value })} value={userObj.password} />
                                <img onClick={togglePasswordVisibility} className={style.hide} src={showPassword ? hide : open} alt="" />
                              </div>
                            </div>

                            <div className={style.pass__field}>
                              <p className={style.input__title}>{passwordMessage ? <span className={style.error}> {passwordMessage} </span> : 'Confirm your password'}</p>
                              <div className={style.input__wrapper}>
                                <input type={showPassword ? 'text' : 'password'} placeholder={passwordConfirmError ? passwordConfirmError : ''} className={passwordConfirmError || passwordMessage || fullPass ? `${style.error__input} ${style.pass__input}` : `${style.pass__input}`} onChange={(e) => setUserObj({ ...userObj, password_confirm: e.target.value })} value={userObj.password_confirm} />
                                <img onClick={togglePasswordVisibility} className={style.hide} src={showPassword ? hide : open} alt="" />
                              </div>
                            </div>
                          </div>
